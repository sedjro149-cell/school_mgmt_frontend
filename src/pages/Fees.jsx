...
const sName = (s) => {
  const fn = s?.user?.first_name || s?.firstname || s?.first_name || "";
  const ln = s?.user?.last_name  || s?.lastname  || s?.last_name  || "";
  return `${fn} ${ln}`.trim() || s?.user?.username || `#${s?.id}`;
};

/** Décode le JWT pour lire is_staff / is_superuser — sans lib externe. */
...
function getIsAdmin() {
  // 1. Essayer localStorage (clé user/profile stockée au login)
  try {
    const profile = localStorage.getItem("user") || localStorage.getItem("profile") || localStorage.getItem("user_data");
    if (profile) {
      const p = JSON.parse(profile);
      if (p.is_staff || p.is_superuser || p.role === "admin") return true;
    }
  } catch {}
  // 2. Lire depuis le JWT (si le backend l'y inclut)
  const token = localStorage.getItem("access_token") || localStorage.getItem("access");
  if (!token) return false;
  const p = decodeJwt(token);
  return !!(p.is_staff || p.is_superuser || p.role === "admin");
}

/* ──────────────────────────────────────────────────────────────
   PAYMENT STATUS HELPERS  (nouveau)
────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   STYLED SELECT  (original avec focus state)
────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   STUDENT CARD  (original + email restauré)
────────────────────────────────────────────────────────────── */
...
const StudentsWithFeesInner = () => {
  ...
  /* ── Arriérés ── */
  const fetchArrears = useCallback(async (studentId) => {
    setLoadingArrears(true);
    try {
      const res = await fetchData(`/fees/arrears/?student=${studentId}`);
      setArrears((res || []).filter((a) => !a.is_settled));
    } catch { setArrears([]); }
  ...
  const openFeesForStudent = async (student) => {
    ...
    setFeesShowAll(true);
    setFeesOpen(true);
    await Promise.all([fetchFees(student.id, false), fetchArrears(student.id)]);
  };
  ...
  /* ── Marquer payé directement (admin) ── */
  const markPaid = async (fee) => {
    setSaving(true);
    try {
      await postData(`/fees/fees/${fee.id}/mark_paid/`, {
        method: "DIRECT",
        note: "Marqué payé par l'administrateur.",
      });
      toast("success", "Frais marqué comme payé.");
      if (selectedStudent) await fetchFees(selectedStudent.id, !feesShowAll);
      fetchStats();
    } catch (err) {
      toast("error", err?.body?.detail || err?.response?.data?.detail || "Erreur lors du marquage.");
    } finally { setSaving(false); }
  };

  /* ── Initialiser fees classe (admin) ── */
  const initializeClassFees = async () => {
    if (!filterClassId) return toast("error", "Sélectionnez une classe.");
    setSaving(true);
    try {
      const res = await postData("/fees/initialize-class/", { class_id: filterClassId });
      toast("success", res?.detail || "Fees initialisés.");
    } catch (err) {
      toast("error", err?.body?.detail || "Erreur initialisation.");
    } finally { setSaving(false); }
  };
  ...
  const fetchPayments = async (feeId) => {
    ...
    try {
      ...
      setPayments((res || []).map((p) => ({
        ...
        // compat backward : si status absent, dériver de validated
        status: p.status || (p.validated ? "approved" : "pending"),
      })));
  ...
  const validatePayment = async (paymentId) => {
    ...
    try {
      ...
      toast("error", err?.body?.detail || "Erreur lors de la validation.");
    } finally { setSaving(false); }
  };
  ...
  const confirmRejectPayment = async () => {
    ...
    try {
      ...
      toast("error", err?.body?.detail || "Erreur lors du rejet.");
    } finally { setSaving(false); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.pageBg, transition: "background .3s",
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "11px 24px",
              ...
              <p style={{ fontSize: 11, color: T.textMuted, marginTop: 1 }}>
                ...
                {activeYear && (
                  <span style={{
                    ...
                    background: `${COL.from}22`, color: COL.from, border: `1px solid ${COL.from}44`,
                  }}>
                    {activeYear.label}
                  </span>
                )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {/* Bouton initialiser fees classe (admin) */}
            {isAdmin && filterClassId && (
              <button onClick={initializeClassFees} disabled={saving}
                title="Créer les fees manquants pour tous les élèves de cette classe"
                style={{
                ...
                <FaSyncAlt style={{ width: 10, height: 10 }} />
                Initialiser frais
              </button>
            )}
            <button onClick={() => { fetchLookups(); fetchStats(); }}
              ...
              {/* Icône tourne si les stats chargent (original) */}
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 24px", display: "grid",
        ...
        {/* ── COLONNE DROITE (sticky) — DESIGN ORIGINAL RESTAURÉ ── */}
        <div style={{ position: "sticky", top: 72, display: "flex", flexDirection: "column", gap: 14 }}>

          {/* Tableau de bord (original : wrapper card avec loading skeleton) */}
          <div style={{
          ...
          {/* Types de frais (design original avec expand/collapse) */}
          <div style={{
                        ...
                        {(ft.amounts || []).map((a) => (
                          <div key={a.id} style={{
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              ...
                              <button onClick={() => {
                                  setCurrentAmount({
                                    id: a.id, fee_type: ft.id,
                                    level: a.level, amount: a.amount, is_active: a.is_active,
                                  });
                                  setAmountOpen(true);
                                }}
                                  style={{
      ...
      <Modal isOpen={feesOpen} onClose={() => setFeesOpen(false)}
        ...
        {selectedStudent && (
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {/* ── Arriérés d'années précédentes (nouveau) ── */}
            {(arrears.length > 0 || loadingArrears) && (
              <div style={{ borderRadius: 12, padding: "12px 16px", background: "#ef444408", border: "1.5px solid #ef444433" }}>
                <p style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: "#ef4444", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                  <FaExclamationTriangle style={{ width: 10, height: 10 }} />
                  Arriérés d'années précédentes
                </p>
                {loadingArrears ? (
                  <div style={{ height: 32, borderRadius: 8, background: "#ef444422" }} className="animate-pulse" />
                ) : arrears.map((a) => (
                  <div key={a.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: 9, background: "#ef444411", border: "1px solid #ef444433", marginBottom: 6 }}>
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 700, color: "#ef4444" }}>{a.fee_type_name || "Frais"}</p>
                      <p style={{ fontSize: 10, color: "#ef444488", marginTop: 1 }}>Depuis {a.source_year_label}</p>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <p style={{ fontSize: 13, fontWeight: 900, color: "#ef4444" }}>{formatCurrency(a.balance)}</p>
            ...
            {/* Formulaire ajout/modif frais (visible pour tous) */}
            <div style={{
              ...
              {feeForm.id && (
                <button onClick={() => setFeeForm({ id: null, fee_type_id: "", amount: "", due_date: "" })}
                  style={{ marginTop: 8, fontSize: 10, color: T.textMuted, background: "none",
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", gap: 4, background: T.inputBg,
                ...
                {[{ label: "Impayés", val: false }, { label: "Tous", val: true }].map(({ label, val }) => (
                  <button key={label}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  ...
                  <tbody>
                    {fees.map((f, idx) => {
                      ...
                      const pendingCount = f.pending_payments_count ?? 0;
                      const even = idx % 2 === 0;
                      return (
                        <tr key={f.id} style={{
                            ...
                            <div style={{ display: "flex", justifyContent: "flex-end", gap: 5 }}>
                              ...
                              {!isPaid && (
                                <button onClick={() => openPaymentsForFee(f)}
                                  title="Encaisser / soumettre paiement"
                                  style={{
                                    width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer",
                                    display: "flex", alignItems: "center", justifyContent: "center",
                              ...
                              {/* Marquer payé directement (admin uniquement) */}
                              {isAdmin && !isPaid && (
                                <button
                                  onClick={() => setConfirm({
                                    open: true,
                                    message: `Marquer "${f.fee_type_name || "ce frais"}" (${formatCurrency(f.total_remaining)}) comme entièrement payé ?`,
                                    onConfirm: () => { setConfirm({ open: false }); markPaid(f); },
                                  })}
                                  title="Marquer payé (admin)"
                                  style={{
                                    width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer",
                                    display: "flex", alignItems: "center", justifyContent: "center",
                                  ...
                                  <FaCheckCircle style={{ width: 10, height: 10 }} />
                                </button>
                              )}
                              {/* Modifier */}
                              ...
                              {/* Supprimer (seulement si aucun paiement) */}
      ...
      {/* ─── MODAL PAIEMENT (z=200) — original + workflow ─── */}
      <Modal isOpen={paymentsOpen} onClose={() => setPaymentsOpen(false)}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Infobande workflow pour non-admin */}
          {!isAdmin && (
            <div style={{ padding: "9px 13px", borderRadius: 10, background: "#3b82f611",
              border: "1.5px solid #3b82f633", display: "flex", alignItems: "flex-start", gap: 8 }}>
              <FaClock style={{ width: 11, height: 11, color: "#3b82f6", flexShrink: 0, marginTop: 1 }} />
              <p style={{ fontSize: 11, color: "#3b82f6", lineHeight: 1.5 }}>
                Votre demande sera soumise en attente de validation par l'administration.
              </p>
            </div>
          )}
          ...
          <button onClick={submitPayment} disabled={saving}
            ...
            {saving ? <FaSyncAlt style={{ width: 13, height: 13 }} className="animate-spin" />
                    : <FaClock style={{ width: 13, height: 13 }} />}
            {saving ? "Traitement…" : (isAdmin ? "Soumettre le paiement" : "Soumettre la demande")}
          </button>
                ...
                {payments.map((p) => (
                  <div key={p.id} style={{
                    ...
                    <div style={{
                      ...
                      {/* Badge status + boutons admin */}
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <PaymentStatusBadge status={p.status} />
                        {isAdmin && p.status === "pending" && (
                          <>
                            <button onClick={() => validatePayment(p.id)} disabled={saving}
                              title="Approuver"
                              style={{
                            ...
                            <button onClick={() => { setRejectModal({ open: true, paymentId: p.id }); setRejectReason(""); }} disabled={saving}
                              title="Rejeter"
                              style={{
                                width: 26, height: 26, borderRadius: 7, border: "none", cursor: "pointer",
                                display: "flex", alignItems: "center", justifyContent: "center",
                    ...
                    {p.status === "rejected" && p.rejection_note && (
                      <div style={{ padding: "5px 12px 8px", borderTop: "1px solid #ef444422", background: "#ef444408" }}>
                        <p style={{ fontSize: 10, color: "#ef4444", fontStyle: "italic" }}>
                          Motif : {p.rejection_note}
                        </p>
                      </div>
                    )}
                  </div>
      ...
      {/* ─── MODAL REJET (z=250, admin) ─── */}
      <Modal isOpen={rejectModal.open} onClose={() => setRejectModal({ open: false, paymentId: null })}
        title="Rejeter ce paiement" icon={FaBan} accentColor="#ef4444" width={400} zIndex={250}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <p style={{ fontSize: 12, color: T.textSecondary, lineHeight: 1.6 }}>
            Indiquez le motif du rejet. Ce message sera visible par l'élève / le parent.
          </p>
          <div>
            <p style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: T.textMuted, marginBottom: 5 }}>
              Motif du rejet
            </p>
            <textarea placeholder="Ex : Montant insuffisant, référence invalide…"
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setRejectModal({ open: false, paymentId: null })}
              style={{ flex: 1, padding: "10px", borderRadius: 10, border: `1.5px solid ${T.cardBorder}`,
                background: "transparent", cursor: "pointer", fontSize: 12, fontWeight: 700, color: T.textSecondary }}>
              Annuler
            </button>
            <button onClick={confirmRejectPayment} disabled={saving}
              style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none",
                background: "linear-gradient(135deg,#ef4444,#dc2626)",
                cursor: saving ? "not-allowed" : "pointer", fontSize: 12, fontWeight: 800, color: "#fff",
